<div class="w-full">
    <div class="flex py-2 {{ $loop->iteration % 2 === 0 ? '' : 'bg-white' }} rounded rounded-xl hover:bg-lime-300">
        ...
        <div class="w-72 px-2 py-2 md:text-lg text-sm text-gray-900 font-bold">{{ $diary->topic }}</div>
        <div class="w-5/12 px-2 py-2 text-sm text-gray-900 font-light md:flex hidden">{!! $diary->description !!}</div>
        <div class="w-5/12 px-2 py-2 text-sm text-gray-900 font-light md:flex hidden">{!! $diary->description !!}</div>
    </div>
</div>